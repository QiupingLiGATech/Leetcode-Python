...
The important part is how to decide move ups and move downs? In this solution, we used flag.  It is either 1 (moving down rows) or -1 (moving up rows). 
When i == 0, we've hit the very top row, so we must start moving down (flag becomes 1). When i == numRows - 1, we've hit the bottom row, so we must start moving up (flag becomes -1).
